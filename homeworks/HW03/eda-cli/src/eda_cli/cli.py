...
def report(
    ...
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
    index_col: Optional[str] = typer.Option(None, help="Колонка для индекса (если есть)."),
    max_hist_columns: int = typer.Option(6, help="Максимум числовых колонок для гистограмм."),
    title: str = typer.Option("report", help="Название отчёта (без формата файла)"),
    ...
    df = _load_csv(Path(path), sep=sep, encoding=encoding, index_col=index_col)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Колонок с превалирующей категорией: **{quality_flags['has_dominant_category']}**\n\n")
