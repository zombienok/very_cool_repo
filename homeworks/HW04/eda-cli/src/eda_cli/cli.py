...
def _load_csv(
    ...
    try:
        return pd.read_csv(path, sep=sep, encoding=encoding, index_col=index_column)
...
def report(
    ...
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
    index_column: Optional[str] = typer.Option(None, help="Колонка для индекса (если есть)."),
    ...
    df = _load_csv(Path(path), sep=sep, encoding=encoding, index_column=index_column)
